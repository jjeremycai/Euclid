...
public struct ModelDownloadFeature {
	...
	public struct State: Equatable {
		...
		var selectedModel: String { euclidSettings.selectedModel }
	...
	private func reduce(state: inout State, action: Action) -> Effect<Action> {
		...
		case let .selectModel(model):
			...
			state.$euclidSettings.withLock { $0.selectedModel = resolved }
		...
		case let .modelsLoaded(recommended, available):
			...
			if state.euclidSettings.selectedModel.contains("*") || state.euclidSettings.selectedModel.contains("?") {
				if let resolved = resolvePattern(state.euclidSettings.selectedModel, from: available) {
					state.$euclidSettings.withLock { $0.selectedModel = resolved }
				}
			}
			...
			updateBootstrapState(&state)
			if !state.anyModelDownloaded && !state.euclidSettings.hasCompletedModelBootstrap {
				let preferred = state.recommendedModel.isEmpty ? state.euclidSettings.selectedModel : state.recommendedModel
				if !preferred.isEmpty {
					state.$euclidSettings.withLock { $0.selectedModel = preferred }
					updateBootstrapState(&state)
				}
			}
			return .none
		...
		case .downloadSelectedModel:
			guard !state.euclidSettings.selectedModel.isEmpty else { return .none }
			state.downloadError = nil
			state.isDownloading = true
			let selected = state.euclidSettings.selectedModel
		...
		case let .downloadProgress(progress):
			...
			if state.downloadingModelName == state.euclidSettings.selectedModel {
		...
		case let .downloadCompleted(result):
			...
			case let .success(name):
				...
				state.$euclidSettings.withLock { settings in
	...
	private func openModelLocationEffect() -> Effect<Action> {
		.run { _ in
			let base = try URL.euclidModelsDirectory
...
extension ModelDownloadFeature.State {
	...
	private var prefersEnglishParakeet: Bool {
		guard let language = euclidSettings.outputLanguage?.lowercased(), !language.isEmpty else {
			return false
		}
