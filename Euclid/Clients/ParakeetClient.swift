...
actor ParakeetClient {
  ...
  private let logger = EuclidLog.parakeet
  private let vendorDirs = [
    // Our app-specific cache path convention (under XDG or com.jjeremycai.Euclid/cache)
    "fluidaudio/Models",
    "FluidAudio/Models"
  ]
...
actor ParakeetClient {
  ...
  func ensureLoaded(modelName: String, progress: @escaping (Progress) -> Void) async throws {
    throw NSError(
      domain: "Parakeet",
      code: -2,
      userInfo: [NSLocalizedDescriptionKey: "Parakeet support not linked. Add Swift Package: https://github.com/FluidInference/FluidAudio.git and link FluidAudio to Euclid."]
    )
  }
